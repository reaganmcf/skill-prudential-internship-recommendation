const languageStrings = {
  'en': {
    translation: {
      SKILL_NAME: 'Prudential Job Finder',
      WELCOME_MESSAGE: 'Welcome back to %s. You can ask for new recommendations, or ask to update your recruitment profile',
      WELCOME_MESSAGE_REPROMPT: 'You can ask for new recommendations, or ask to update your recruitment profile',
      WELCOME_MESSAGE_NO_PROFILE: 'Welcome to %s. You can ask a question like, tell me more about prudential, or build my personal recruitment profile.',
      WELCOME_MESSAGE_NO_PROFILE_REPROMPT: 'You can ask a question like, tell me more about prudential, or build my personal recruitment profile.',
      GENERAL_INFORMATION: 'With $3.7 trillion in life insurance policies in force worldwide and $1.334 trillion in assets under management. Prudential is the 2nd-largest life insurer in the United States and the 9th-largest institutional asset manager worldwide. Prudential consistently ranks at the top of FORTUNE® Magazine’s Most Admired Companies in the World and as one of Pension & Investments’ Top Money Managers. We are also proud of the recognition that we have earned from a variety of publications and associations for our workplace policies and contributions to the community. Our diverse and talented employees in offices around the world are committed to helping people and companies attain financial wellness through our innovative products and financial solutions. Whether working with multinational corporations or individual families, we are dedicated to helping create financial strength today and security for tomorrow.  Would you like to here about internships and jobs opportunities?',
      GENERAL_INFORMATION_REMPROMPT: 'Would you like to here about internships and jobs opportunities?',
      GENERAL_INFORMATION_ABOUT_INTERNSHIPS: 'some general info about internships. say search for jobs to find some internship opportunities',
      GENERAL_INFORMATION_ABOUT_INTERNSHIPS_REPROMPT: 'say search for jobs to find some internship opportunities',
      SAVED_PROFILE: 'Successfully saved your personal recruitment profile. Would you like to view applications that fit your profile?',
      SAVED_PROFILE_REPROMPT: 'Would you like to view applications that fit your profile?',
      HELP_MESSAGE: 'You can ask questions such as, update my recruitment profile, or tell me more about prudential',
      HELP_REPROMPT: 'You can ask questions such as, update my recruitment profile, or tell me more about prudential ... Now, what can I help you with?',
      STOP_MESSAGE: 'Goodbye!',
      ERROR_MESSAGE: 'An error occurred. Please try again'
    },
  }
};

module.exports = languageStrings;